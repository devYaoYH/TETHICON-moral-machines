...
```
bazel run //config_tools:create_sample -- -output_dir=''

bazel run //config_tools:config_reader -- -input_path='job.json' -config_type=job

bazel run //runner:step_runner -- -input_path='step.json'

bazel run //runner:task_runner -- -input_path='task.json'
```
