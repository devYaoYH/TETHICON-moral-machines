...
PreprocessProfiles <- function(profiles){
  ...
  profiles[,AttributeLevel := factor(AttributeLevel, 
                                    levels=c("Rand",
                                             ...
                                             "Less","More",
                                             "Pets","Hoomans"))]
  profiles[,Intervention := factor(Intervention, levels=c(0,1))]
  ...
  profiles$consistent <- ifelse(profiles$consistent == "", NA, tolower(profiles$consistent))
  profiles$consistent <- ifelse(profiles$consistent %in% c("true", "1", "TRUE"), TRUE,
                                ifelse(profiles$consistent %in% c("false", "0", "FALSE"), FALSE, NA))
  
  profiles$coherent <- ifelse(profiles$coherent == "", NA, tolower(profiles$coherent))
  profiles$coherent <- ifelse(profiles$coherent %in% c("true", "1", "TRUE"), TRUE,
                                ifelse(profiles$coherent %in% c("false", "0", "FALSE"), FALSE, NA))
...
GetMainEffectSizes <- function(profiles,savedata,r){
  ...
  # For intervention (commented out for now)
